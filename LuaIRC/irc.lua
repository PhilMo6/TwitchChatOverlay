...
require "LuaIRC.util"
require "LuaIRC.asyncoperations"
require "LuaIRC.handlers"
...
print('repeat')
	...
	until (self.authed or shutdownNow)
end
...
local function getline(self, errlevel)
	local line, err = self.socket:receive("*l")
		...
		if line then print(line) elseif err and err ~= "timeout" and err ~= "wantread" then print(err) end
	...
	if line and string.find(line, "Improperly formatted auth") then
		shutdownNow = true
		self:shutdown()
		print("#@#!#!#")
		error(err, errlevel)
